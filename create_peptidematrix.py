...
def mainPSM(myPath, result_file):
    ...
    def proteomeDiscoverer(my_file):
        ...
        with open(my_file, "r") as f:
            next(f)  # skip first line
            for line in f:
                peptide = line.split("\t")[4].split(".")[1].upper().rstrip().replace("I", "J").replace("L", "J")
                peptideList.append(peptide)
    ...
    def COMPIL(my_file):
        peptideList = list()  # in COMPIL files here are only unique peptides
        with open(my_file, "r") as f:
            for line in f:
                peptide = line.rstrip().replace("I", "J").replace("L", "J")
                peptideList.append(peptide)
    ...
    for se in sample_db:
        ...
        for result in os.listdir(myPath + "/" + se):
            ...
            if se == "MQ":
                ...
                peptideList = COMPIL(myPath + "/" + se + "/" + result)
            else:
                print("Are you sure?")
